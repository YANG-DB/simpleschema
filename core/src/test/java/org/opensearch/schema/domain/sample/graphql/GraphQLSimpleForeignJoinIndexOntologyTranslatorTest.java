...
//    @Ignore("Fix overriding properties by different schema types with similar names")
    ...
    public void testBookEntityTranslation() {
        ...
        Assertions.assertEquals(1,ontologyAccessor.entity$("Book").getDirectives().size());
        Assertions.assertEquals(new DirectiveType(MODEL.name().toLowerCase(), DirectiveType.DirectiveClasses.DATATYPE),ontologyAccessor.entity$("Book").getDirectives().get(0));
    }
